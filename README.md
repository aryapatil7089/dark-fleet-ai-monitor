...
**Track:** Data Science Portfolio Project
...
**[👉 CLICK HERE TO WATCH THE CLOUD DEPLOYMENT & UI DEMO](https://drive.google.com/drive/folders/1kOXWMeAoVwea6Wxq3DKrlmb0L9k8O_HX?usp=drive_link)**
...
## 💾 Model Training & Dataset Origin
To train the baseline physics and clustering thresholds of the Machine Learning models, we utilized historical Automatic Identification System (AIS) broadcast data collected by the U.S. Coast Guard. 
* **Original Source:** The raw training data originates from the **MarineCadastre.gov** project, a joint initiative by the Bureau of Ocean Energy Management (BOEM) and NOAA's Office for Coastal Management. 
* **Data Scale:** The original uncompressed source file (`2024_NOAA_AIS_logs_01.parquet`) contains over 3.15 GB of spatial maritime telemetry.
* **Downsampling for Portability:** For the purpose of this portfolio repository and to ensure local execution speed for reviewers, the original dataset was strategically downsampled to a lightweight **45 MB** parquet file. This sample retains the statistical distributions necessary to successfully run the Jupyter Notebook and fit the `StandardScaler` and `K-Means` models without exceeding GitHub's 100 MB file limit.
...
1. **Dynamic Bounding Boxes:** Dropdown selector to switch monitoring zones (Singapore, English Channel, Gulf of Mexico, Global) on the fly. This intentionally severs and rebuilds the satellite connection to prevent the browser from rendering the entire ocean at once and freezing.
